...
import org.bhc.common.overlay.message.MessageCodec;
import org.bhc.common.overlay.message.StaticMessages;
import org.bhc.core.db.ByteArrayWrapper;
import org.bhc.core.exception.P2pException;
import org.bhc.core.net.TronNetHandler;
...
public class Channel {
  ...
  private InetSocketAddress inetSocketAddress;  //远程对方地址
  ...
  public void publicHandshakeFinished(ChannelHandlerContext ctx, HelloMessage msg) {
    ...
    msgQueue.activate(ctx);   //只有激活后才能广播区块消息
    ctx.pipeline().addLast("messageCodec", messageCodec);  //识别消息解析成TronMessage、p2pMessage两类消息
    ctx.pipeline().addLast("p2p", p2pHandler);       //处理p2pMessage消息
    ctx.pipeline().addLast("data", tronNetHandler);  //处理TronMessage
