package org.bhc.common.overlay.discover.node.statistics;
...
public class MessageCount {
  ...
  private int index = (int) (indexTime % SIZE); //取值0-59，
  ...
  private void update() {
    ...
    int k = gap > SIZE ? SIZE : (int) gap;  //限制时间间隔最大为60s
    if (k > 0) {
      for (int i = 1; i <= k; i++) {
        szCount[(index + i) % SIZE] = 0;  //初始化数组
      }
      index = (int) (time % SIZE);   
      indexTime = time;
    }
