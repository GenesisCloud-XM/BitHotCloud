...
import org.bhc.protos.Protocol.Account.AccountResource;
import org.bhc.protos.Protocol.Account.Frozen;
...
public class  AccountCapsule implements ProtoCapsule<Account>, Comparable<AccountCapsule> {
  ...
  /**
   * initial account capsule. 根据参数构造出Account对象
   */
  ...
  /**
   * construct account from AccountCreateContract.  根据账户创建合约构造出Account对象
   */
  ...
  /** 根据地址、类型、创建时间、许可权限、管理者 构造出Account对象
   * get account from address.
   */
  public AccountCapsule(ByteString address,
