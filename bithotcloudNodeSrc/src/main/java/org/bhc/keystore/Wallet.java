...
public class Wallet {
  ...
  public static WalletFile create(String password, ECKey ecKeyPair, int n, int p)
	  ...
	  //生成 32 位随机字节数组 salt 对用户密码进行加密
    byte[] salt = generateRandomBytes(32);

    // 加密用户密码
    byte[] derivedKey = generateDerivedScryptKey(password.getBytes(UTF_8), salt, n, R, p, DKLEN);
    byte[] encryptKey = Arrays.copyOfRange(derivedKey, 0, 16);  //取前16字节作为加密密钥
  ...
  private static WalletFile createWalletFile(
    ...
    walletFile.setAddress(org.bhc.core.Wallet.encode58Check(ecKeyPair.getAddress()));
  ...
  private static byte[] generateDerivedScryptKey(
    ...
    return SCrypt.generate(password, salt, n, r, p, dkLen);
     // SCrypt 是一种针对密码的加密方法, 参考 wiki: https://en.wikipedia.org/wiki/Scrypt
  }
